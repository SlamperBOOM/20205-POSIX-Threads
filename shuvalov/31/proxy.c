...
//                    printf("buf_len = %zd\n", servers[server_index].request->buf_len);
                }
            }
        }
    }
    ...
    for (int i = 0; i < clients_size; i++) {
        ...
        if (client->fd != -1 && !client->processed) {
            if ((poll_fds[client->poll_index].revents & POLLIN) == POLLIN) {
                receive_from_client(client, i, poll_fds, poll_fds_num, clients, clients_size, servers, servers_size);
...
int send_to_server(struct server* server, struct pollfd* poll_fds, size_t poll_fds_num, struct client* clients,
                   size_t clients_size,
                   struct server* servers, size_t servers_size) {
...
int receive_from_server(struct server* server, int server_index, struct pollfd* poll_fds, size_t poll_fds_num,
    ...
    if (return_value < 0) {
        ...
        for (int k = 0; k < server->clients_num; k++) {
            printf("Client POLLOUT\n");
            add_fd_to_poll(server->clients[k]->fd, POLLOUT, poll_fds, poll_fds_num);
            remove_from_poll(server->fd, poll_fds, poll_fds_num);
            setup_server(server_index, servers);
        }
    } else {
...
int process_servers(struct pollfd* poll_fds, size_t poll_fds_num, struct client* clients, size_t clients_size,
                    struct server* servers, size_t servers_size) {
    for (int i = 0; i < servers_size; i++) {
        ...
        if (server->fd != -1 && !server->processed) {
            if ((poll_fds[server->poll_index].revents & (POLLOUT | POLLHUP)) == (POLLOUT | POLLHUP)) {
                ...
                send_to_server(server, poll_fds, poll_fds_num, clients, clients_size, servers, servers_size);
            } else if ((poll_fds[server->poll_index].revents & POLLIN) == POLLIN) {
                receive_from_server(server, i, poll_fds, poll_fds_num, clients, clients_size, servers, servers_size);
            }
        }
    }
    return EXIT_SUCCESS;
...
int main(int argc, char* argv[]) {
    ...
    while (iter++ < 100) {
        ...
        if (process_clients(poll_fds, poll_fds_num, clients, clients_size, servers, servers_size) == 1) {
            fprintf(stderr, "process_clients failed\n");
        }
        process_servers(poll_fds, poll_fds_num, clients, clients_size, servers, servers_size);
        for (int i = 0; i < poll_fds_num; i++) {
            servers[i].processed = 0;
